import { Heart, Trash2, ArrowLeft, Play } from "lucide-react";
        ...
        <div className="flex flex-col gap-6 px-6 pt-10 pb-8 sm:flex-row sm:items-center sm:justify-between sm:px-12 lg:px-20">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="font-heading text-[10px] font-black uppercase tracking-[0.3em] text-primary">Intelligence</span>
            </div>
            <h1 className="font-heading text-3xl font-black text-white uppercase tracking-tighter italic">Watchlist</h1>
            <p className="font-body text-xs text-white/40 italic">Pinned coordinates for future extraction</p>
          </div>
          {watchlist.length > 0 && (
            <button
            ...
            >
              <Trash2 className="h-4 w-4" /> Reset List
            </button>
          )}
        </div>
        ...
        {watchlist.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-40 animate-fade-in">
            <div className="relative mb-8">
              ...
              <Heart className="relative h-20 w-20 text-white/10" />
            </div>
            <h3 className="font-heading text-xl font-black text-white uppercase tracking-tight">Intelligence Empty</h3>
            <p className="mt-4 font-body text-sm text-white/20 italic">Mark your targets across the cinematic landscape</p>
            <button
            ...
            >
              <ArrowLeft className="h-4 w-4" /> Find Content
            </button>
          </div>
        ) : (
            ...
            {watchlist.map(entry => (
              ...
              >
                <div className="relative aspect-[2/3] overflow-hidden rounded-[24px] border border-white/5 bg-white/[0.03] transition-all duration-500 hover:border-primary group-hover:shadow-[0_0_30px_rgba(229,9,20,0.2)]">
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); removeFromWatchlist(entry.item.id); }}
                ...
                <div className="px-1">
                  ...
                  <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest italic">{entry.mediaType === 'tv' ? 'Series' : 'Feature'}</p>
                </div>
              </div>
            ))}
          </div>
